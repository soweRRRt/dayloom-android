    ...
    private fun scheduleNextOccurrence(
        ...
        val weekdays =
            inputData
                ...
                ?.toSet()
                .orEmpty()
        if (minutes !in 0 until 24 * 60 || weekdays.isEmpty()) return

        val now = System.currentTimeMillis()
        val nextTrigger = nextWeeklyTrigger(now, weekdays, minutes)
        val next =
            ScheduledNotification(
                ...
                title = title,
                weeklyRepeat = WeeklyNotificationRepeat(weekdays, minutes),
            )
        WorkManager
    ...
    OneTimeWorkRequestBuilder<ReminderWorker>()
        ...
        .setInputData(
            Data
                ...
                .putInt(KEY_REPEAT_MINUTES, weeklyRepeat?.minutesOfDay ?: NO_REPEAT)
                .build(),
        ).addTag(REMINDER_TAG)
